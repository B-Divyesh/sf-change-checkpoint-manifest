...
| F-1-2 | Added `runs-in-git-repository` and `local-signing-key-path` to `claims.json`. Each has one tagged observable browser/CLI test using a caller-created fresh Git repository. | `@claim:runs-in-git-repository`; `@claim:local-signing-key-path`; both ran from fresh clone `/tmp/change-checkpoints-clean-MRbQ55`. |
